...
$.ajax({
          ...
          success: function(data) {
            var users = JSON.parse(JSON.stringify(data));
               $("#detail").
                append('<input type="text" id="names" required="" class="form-control" name="" value="'+users.name+'" placeholder="Nama Lengkap" >\
                      <br>\
                      <input type="text" id="emails" required=""  class="form-control"  value="'+users.email+'" name="" placeholder="Email">\
                      <br>\
                      <input type="text" id="username" required="" class="form-control"  name="" value="'+users.username+'" placeholder="Username">\
                      <br>\
                      <input type="text" id="telepon" class="form-control" value="'+users.telepon+'"  name="" placeholder="Telepon">\
                      <br>\
...
function test()
{
    ...
    $.ajax({
              type:"PUT",
              url:"http://localhost:9081/user/updateProfile/"+name+"/"+email+"/"+telepon+"/"+username+"/"+alamat+"/"+id,
